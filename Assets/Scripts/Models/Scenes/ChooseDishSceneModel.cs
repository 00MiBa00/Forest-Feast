...
namespace Models.Scenes
{
    public class ChooseDishSceneModel
    {
        ...
        public List<DishPreview> GetRecipeDishes(List<RecipeSO> recipeSos, List<Sprite> iconSprites)
        {
            List<DishPreview> dishes = new List<DishPreview>();
            
            dishes.AddRange(GetRecipeSOToDishPreviewConverter(recipeSos));
            dishes.AddRange(GetJsonToDishPreviewConverter(iconSprites));
        ...
        private List<DishPreview> GetRecipeSOToDishPreviewConverter(List<RecipeSO> recipeSos)
        {
            return recipeSos
                .Select(recipe => new DishPreview(recipe.Title, recipe.Icon))
                .ToList();
        }

        private List<DishPreview> GetJsonToDishPreviewConverter(List<Sprite> iconSprites)
        {
            ...
            return block.Recipes
                .Select(r => new DishPreview(r.Tile, iconSprites[r.IconIndex]))
                .ToList();
        }
    }
}